...
contract MockAllocationManager is IAllocationManager, Test {
    ...
    mapping(address => bool) public authorizedSlashers; // Allow attack contracts to slash
    ...
    function slashOperator(
        ...
        require(msg.sender == slashers[operatorSetKey] || authorizedSlashers[msg.sender], "Invalid slasher");
        ...
        for (uint256 i = 0; i < params.strategies.length; i++) {
            ...
            // Apply pending changes first (like real contract's _getUpdatedAllocation)
            if (allocation.effectBlock != 0 && block.number >= allocation.effectBlock) {
                allocation.currentMagnitude = uint64(uint128(int128(uint128(allocation.currentMagnitude)) + allocation.pendingDiff));
            ...
            // If there's a pending deallocation, reduce it proportionally (like real contract)
            if (allocation.pendingDiff < 0) {
                uint64 slashedPending = uint64((uint256(uint128(-allocation.pendingDiff)) * params.wadsToSlash[i] + 1e18 - 1) / 1e18);
    ...
    function modifyAllocations(
        ...
        for (uint256 i = 0; i < params.length; i++) {
            ...
            for (uint256 j = 0; j < params[i].strategies.length; j++) {
                ...
                require(allocation.effectBlock == 0, "Modification already pending");
                ...
                if (pendingDiff < 0) {
                    ...
                    // For simplicity, apply allocation immediately (real contract has delay)
                    allocation.currentMagnitude = params[i].newMagnitudes[j];
                    allocation.pendingDiff = 0;
                    allocation.effectBlock = uint32(block.number);
                }
    ...
    function getAllocation(
        ...
        // Apply pending changes if effectBlock has passed (like real contract's _getUpdatedAllocation)
        if (allocation.effectBlock != 0 && block.number >= allocation.effectBlock) {
            allocation.currentMagnitude = uint64(uint128(int128(uint128(allocation.currentMagnitude)) + allocation.pendingDiff));
            allocation.pendingDiff = 0;
            allocation.effectBlock = 0;
    ...
    function setAllocation(
        ...
        allocations[operator][operatorSetKey][strategy].pendingDiff = 0;
